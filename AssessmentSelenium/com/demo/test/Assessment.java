...
public class Assessment {
	private static int xlRows, xlCols;
	...
	public void operation() throws Exception {
		...
		for (int i = 1; i < xlRows; i++) {
			if (xData[i][1].equalsIgnoreCase("Y")) {
				...
				// Starting search while the column says Y
				String vSearch = xData[i][0];
				System.setProperty("webdriver.chrome.driver", "C:\\Selenium\\chromedriver.exe");
				WebDriver driver = new ChromeDriver();
				driver.manage().window().maximize();
				driver.get("https://www.yahoo.com/");
				driver.findElement(By.xpath("//*[@id=\"uh-search-box\"]")).sendKeys(vSearch, Keys.ENTER);
				String vResult = driver.getTitle();
				Thread.sleep(2000);
				driver.close();
				System.out.println(vResult);

				// Getting Web page title
				xData[i][2] = vResult;

				// Writing results onto the excel sheet
				xlwrite("C:\\Selenium\\testResults.xls", xData);
